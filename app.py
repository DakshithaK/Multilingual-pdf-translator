...
import os
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = 'Path to your Service account JSON credentials file'
...
def extract_text(file_path, source_lang):
    ...
    all_t = ""
    for page in pages:
      filename = "page_" + str(image_counter)
      st.write(filename)
...
def translate_row(text, target_lang):
    target_lang_code = language_codes_target[target_lang]
    #translator = google_translator()
    output = translate_client.translate(text, target_language=target_lang_code)
